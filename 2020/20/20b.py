...
def load_tiles(fn):
    raw_tiles = [g for g in open(fn, 'r').read().split('\n\n')]
    tiles = {}
...
def rotflip_rev(arr, rf):
    for f in range(rf[1]):
        arr = np.flip(arr, axis=1)
    arr = np.rot90(arr, 4 - (rf[0] % 4))
    return arr


# def rotflipflip(arr, rff):
#     arr = np.rot90(arr, rff[0])
#     for _ in range(rff[1]):
#         arr = np.flip(arr, axis=0)
#     for _ in range(rff[2]):
#         arr = np.flip(arr, axis=0)
#     return arr
...
def place_tiles(G, tiles, tile_locs, start):
    ...
    # for _ in range(10):
        ...
        if t1 in tile_locs.values():
            ...
            # place new tile
            rel_loc = rel_tile_locs[(edge_o[t2][0])]
            ...
            # get matched edge
            # reverse needed rotflip so that top string of t2 is the match
            temp_tile = rotflip(tiles[t2], (o2[0], o2[1]))
            match_edge = ''.join(temp_tile[0])  # top string
            # print(match_edge)
            # print(tile_str(temp_tile))
            # spin and flip new tile so that bottom string of t1 is the match
            for r in range(4):
                for f in range(2):
            ...
            # print(tile_str(updated_tile))

            updated_tile = rotflip_rev(updated_tile, (o2[0], o2[1]))
            # print(tile_str(tiles[t2]))
            # print(tile_str(updated_tile))
            # updated_tile = rotflip_rev(updated_tile, (o1[0], o1[1]))
            
            # updated_tile = rotflip(updated_tile, (o1[0], o1[1]))
            
            tiles[t1] = updated_tile


            # print(f"from: {t2} to: {t1}, new_loc: {new_loc}")
            # print(f"o1o2uw: {o1, o2, uw}, fliprot: {flip, rot}\n")
...
def combine_tiles(tiles, tile_locs, edge_length=10):
    ...
    for tl, tile_n in tile_locs.items():
        ...
        # print(out_arr[ty:ty + edge_length, tx:tx + edge_length])
